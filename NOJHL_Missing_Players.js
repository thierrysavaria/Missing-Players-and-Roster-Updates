function appendMissingPlayersNOJHL() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const nojhlSheet = ss.getSheetByName("NOJHL");
  const rosterSheet = ss.getSheetByName("Roster_Update_NOJHL");
  ...
  const nojhlData = nojhlSheet.getDataRange().getValues();
  ...
  const nojhlPlayers = new Set(nojhlData.map(row => row[4]?.toString().trim().toLowerCase())); 
