function appendMissingPlayersOJHL() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ojhlSheet = ss.getSheetByName("OJHL");
  const rosterSheet = ss.getSheetByName("Roster_Update_OJHL");
  ...
  const ojhlData = ojhlSheet.getDataRange().getValues();
  ...
  const ojhlPlayers = new Set(ojhlData.map(row => row[4]?.toString().trim().toLowerCase())); 
