function appendMissingPlayersAJHL() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ajhlSheet = ss.getSheetByName("AJHL");
  const rosterSheet = ss.getSheetByName("Roster_Update_AJHL");
  ...
  const ajhlData = ajhlSheet.getDataRange().getValues();
  ...
  const ajhlPlayers = new Set(ajhlData.map(row => row[4]?.toString().trim().toLowerCase())); 
