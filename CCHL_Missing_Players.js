function appendMissingPlayersCCHL() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const cchlSheet = ss.getSheetByName("CCHL");
  const rosterSheet = ss.getSheetByName("Roster_Update_CCHL");
  ...
  const cchlData = cchlSheet.getDataRange().getValues();
  ...
  const cchlPlayers = new Set(cchlData.map(row => row[4]?.toString().trim().toLowerCase())); 
